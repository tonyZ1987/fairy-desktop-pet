...
| 情形 | 表现 |
|---|---|
| **刚启动** | **常态**（睁眼）—— 上一轮的工作态**不会**被新进程继承 |
| **你发来一条消息** | 立刻**半睁、无进度条**，然后**一直保持** |
| **你等它的时候**（它在想事情） | **保持半睁** —— 哪怕它几分钟不调工具、不写一个字 |
| **它把话说完** | 约 **25 秒**后自己回常态；**你一开始在输入框打字**，它**立刻**回常态 |
| **它报了进度** | 半睁 + **进度条**（按最小 1% 的台阶往上爬） |

一句话：**你一发消息它就半睁，一直到它把话说完** —— 中间不闪、不提前睁眼。

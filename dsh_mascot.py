...
GLOW_COVER_V = 0.10         # ★ 扫描线"满强度"的亮度阈值（v = max 通道 / 255）
# ★★ 2026-09-22 主人："常态的水平摩尔纹在白底上看，最边缘的消逝还是有点生硬"。
#   根因：原来是 `clip(v / 0.02)` —— 只要辉光还剩 5/255，扫描线就**已经是满强度**，
#   随后在最后 4~5 px 里跟着辉光一起掉到 0 ⇒ 观感就是"撑到最外一条纹、戛然而止"。
#   现在改成 `smoothstep(v / 0.10)`：满强度只保留到 rho≈1.35，外面留出
#   rho 1.35 → 1.52（260px 档约 18 px）的**平滑渐薄**，且两端导数都为 0（无折点）。
#   ⚠️ 改这里必须 `fairy_layers.BAKE_VERSION` +1 并删 `_cache/` 重烘（scan_a 是烘焙产物）。


def glow_cover(x, y):
    """扫描线遮罩（0..1）—— 原素材里扫描线只出现在辉光区内。"""
    v = glow_layer(x, y).max(axis=2) / 255.0
    return smoothstep(np.clip(v / GLOW_COVER_V, 0.0, 1.0))


# ---------- ★ 工作态外缘收缩（主人 2026-09-22："工作态边缘还是挺明显的，把边缘往里收点"）----------
#   量测依据（260px 白底、盘正上方外缘带）：工作态在 rho 1.26~1.46 那段比常态**亮 1~2.5 级**
#   （底光 + 淡雾叠出来的），所以"到白底那一步"更实、边界更明显。常态已经改好，工作态没有。
#   这里给工作态三层（L0 底光 / L1 淡雾 / L2 摩尔纹）加一个**外缘收缩包络**。
#   ★ 数字层（L3 的 0/1）**不收**——主人明确说"哪怕少量数字溢出底面也能接受"。
#   ★★ 这个包络**在载入缓存后做**（`fairy_layers._apply_work_cut`），**不用重烘**：
#      它是对烘焙结果做逐像素径向乘，而 alpha 查表也是逐像素函数 ⇒ 与"烘进层里"等价。
#      ⇒ **调这几个数只要双击 restart_fairy.vbs。**
#   ★★★ 2026-09-22 第二轮：第一版用 smoothstep 窗（1.15→1.38）被主人判为"硬裁，不是衰减型收束"
#      —— 对，因为它在 1.30 处还有 6~7 级墨、到 1.38 直接归零，**9 px 内掉完**。
#      现在改**指数衰减尾** `exp(-((rho-IN)/TAU)^2)`：渐近趋 0、没有终点，
#      等于把"一段被裁掉的尾巴"换成"越来越淡的雾"。实测最陡斜率 3.1 → 1.3 级/px。
WORK_CUT_MODE = "exp"       # "exp" = 纯指数尾（现用）｜"gauss" = 高斯尾 ｜"step" = smoothstep 窗（旧，硬裁）
WORK_CUT_IN = 1.00          # 衰减从哪个 rho 开始（以内不动）
WORK_CUT_TAU = 0.30         # ★ 衰减尺度（主人 2026-09-22 看完黑底版 MP4 后定：取整 0.30）
#   越大尾巴越长、外缘越软（可见外缘 ≈ IN + 1.4·TAU）。改这个只要重启，**不用重烘**。
WORK_CUT_OUT = 1.38         # 仅 "step" 模式用（模型后期若切回窗式）
#   实测（260px 白底，`_work/step24_cut_shape.py`）—— "末段" = 墨量从 5 级降到 0.5 级要走多少 px：
#     A 不收            可见外缘 1.503 ｜ 1.30 处 17.8 级 ｜ 末段 10 px ｜ 0.52 级/px
#     B 旧 smoothstep   可见外缘 1.332 ｜ 1.30 处  2.4 级 ｜ 末段  8 px ｜ 0.59 级/px ← 主人判"硬裁"
#     C 指数尾 τ=0.22   可见外缘 1.422 ｜ 1.30 处  4.2 级 ｜ 末段 17 px ｜ 0.29 级/px ← **现默认**
#     D 指数尾 τ=0.36   可见外缘 1.467 ｜ 1.30 处  7.5 级 ｜ 末段 16 px ｜ 0.31 级/px
#   ⇒ 指数尾把「末段」拉长一倍、斜率砍半：同样是收，但收得「越来越淡」而不是「被切一刀」。
#   定稿 τ=0.30（取整）实测 —— `_work/step29_tau030.py`：
#     白底：可见外缘 1.422 ｜ 1.30 处 6.27 级 ｜ 末段 17 px
#     黑底：可见外缘 1.350 ｜ 1.30 处 2.27 级 ｜ 末段 **19 px（四档里最长 ⇒ 最软）**
#   对照：τ=0.22 → 白 1.395/末段 17 ｜ τ=0.36 → 白 1.440/末段 16 ｜ τ=0.50 → 白 1.449/末段 14


def work_cut_active():
    """是否启用外缘收缩（"step" 模式下把 IN≥OUT 当作关闭）。"""
    return not (WORK_CUT_MODE == "step" and WORK_CUT_IN >= WORK_CUT_OUT)


def work_cut(rho):
    """工作态外缘收缩包络（1 → 0，单调、渐近、无折点）。"""
    r = np.asarray(rho, dtype=np.float64)
    if WORK_CUT_MODE == "step":
        u = np.clip((r - WORK_CUT_IN) / max(1e-6, WORK_CUT_OUT - WORK_CUT_IN), 0.0, 1.0)
        return 1.0 - smoothstep(u)
    u = np.clip(r - WORK_CUT_IN, 0.0, None) / max(1e-6, WORK_CUT_TAU)
    if WORK_CUT_MODE == "gauss":
        return np.exp(-u * u)
    return np.exp(-u)          # "exp"：纯指数尾（渐近、永远不为 0 ⇒ 没有"切一刀"的终点）
